...
## P5 — Editing + export + reconciliation + backups — **server half done** (2026-07-15; editor UI pending mockup approval)

Editor/library mockup published for approval (2026-07-15):
`docs/mockups/p5-editor.html` (artifact
claude.ai/code/artifact/10b4f9c8-e983-46c3-86d0-19871b275d07): editor with
raw-first structured ingredient rows (parsed/check/no-amount chips,
expandable amounts panel, re-parse + manual-lock), bulk paste modal,
favorites + private notes, Settings → Library (rescan report, backups),
mobile variant. Flutter implementation starts after approval.
